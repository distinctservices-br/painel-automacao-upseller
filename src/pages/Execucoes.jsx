...
function MetricCard({ label, value, trend, tone, accent }) {
  ...
  return (
    <div className="glass-card p-5 flex flex-col gap-1.5">
      ...
      <p className={`font-display font-bold text-[30px] tracking-tight leading-none ${accent ? 'text-primary' : 'text-white-1'}`}>
        {value}
      </p>
...
export default function Execucoes() {
  ...
  // Últimos 7 dias como labels para o gráfico
  ...
  return (
    <>
      ...
      {/* Filtros */}
      <div className="flex flex-wrap items-center gap-2.5 px-4 py-3 mb-5 bg-black-2 border border-divider rounded-lg">
        <span className="text-[11px] uppercase tracking-[0.08em] text-muted-light mr-1">Cliente</span>
        <select
          value={clienteId}
        ...
        <div className="flex bg-surface border border-[rgba(250,250,250,0.08)] rounded-full p-[3px] gap-0.5">
          {PERIODOS.map((p) => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-[12px] font-body transition-all duration-150
      ...
      <div className="bg-black-2 border border-divider rounded-lg overflow-hidden mb-5">
        <div className="flex justify-between items-center px-5 py-3.5 border-b border-divider">
          <p className="font-display font-semibold text-[14px] tracking-tight">
            Corridas — {PERIODOS.find((p) => p.id === periodo)?.label}
          </p>
          <span className="font-mono text-[11px] text-muted">{execucoes.length} registros</span>
        </div>
        {loading ? (
          <div className="flex items-center justify-center py-12 text-muted">
            <i className="ti ti-loader-2 animate-spin text-xl mr-2" /> Carregando…
          </div>
        ) : execucoes.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-muted gap-2">
            ...
            <p className="text-[13px]">Nenhuma execução encontrada para o período.</p>
          </div>
        ) : (
            ...
            <thead>
              <tr>
                {['Horário', 'Cliente', 'Loja', 'Pedidos', 'Status'].map((h) => (
                  <th key={h} className="text-left px-5 py-2.5 font-medium text-[11px] uppercase tracking-[0.07em] text-muted-light bg-[rgba(250,250,250,0.02)] border-b border-divider">
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {execucoes.map((r) => (
                <tr key={r.id} className="border-b border-divider hover:bg-[rgba(115,243,164,0.03)] transition-colors">
                  <td className="px-5 py-3 font-mono text-[12px] text-muted">
                    {new Date(r.data_execucao).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                  </td>
                  <td className="px-5 py-3">{r.nome_cliente}</td>
                  <td className="px-5 py-3">
                    <span className="inline-flex px-2 py-0.5 rounded-[6px] bg-muted-surface border border-divider font-mono text-[11px] text-muted">
                      {r.nome_loja} · {r.shop_id}
                    </span>
                  </td>
                  <td className="px-5 py-3 font-mono text-white-1">{r.total_impresso ?? 0}</td>
      ...
      {porLoja.length > 0 && (
        <div className="bg-black-2 border border-divider rounded-lg px-6 py-5">
          <div className="flex justify-between items-baseline mb-4">
            <p className="font-display font-semibold text-[14px] tracking-tight">Volume por loja</p>
            <span className="font-mono text-[11px] text-muted">últimos 7 dias</span>
          </div>
          <div className="flex flex-col gap-3.5">
            {porLoja.map((v) => {
              const bars = ultimos7Dias.map((d) => v.dias[d] ?? 0)
              return (
                <div key={v.loja_id} className="grid grid-cols-[1fr_50px] gap-3 items-center">
                  <div>
                    <div className="flex justify-between text-[12px] text-muted mb-1.5">
                      <span>{v.nome_loja}</span>
                      <span className="font-mono text-muted-light text-[11px]">seg · ter · qua · qui · sex · sáb · dom</span>
                    </div>
                    <Bars values={bars} />
                  </div>
